...
NOVO:
- Converte Markdown simples -> DOCX (títulos, listas, separadores, **negrito**)
- Insere tabelas no DOCX quando o dataframe tiver poucas colunas
- Insere gráficos gerados (Plotly JSON) no DOCX como imagem PNG (requer plotly+kaleido)
"""
                    ...
                    analysis["assistant_responses"].append(
                        {
                            ...
                            "chart": content.get("chart"),  # <-- NOVO: guardar o chart JSON
                        ...
                        analysis["queries_executed"].append(
                            {
                                ...
                                "chart": content.get("chart"),  # <-- NOVO (opcional): chart por consulta
                            }
                        )
...
"""
        ...
        try:
            ...
            # vanna.config pode não existir dependendo do setup — protege
            model_name = "gpt-oss:20b"
            try:
                model_name = (getattr(self.vanna, "config", {}) or {}).get("model", model_name)
            except Exception:
                pass
    ...
    # ---------------------------------------------------------------------
    # Plotly JSON -> PNG -> DOCX
    # ---------------------------------------------------------------------
    def _chart_json_to_png_bytes(self, chart_json: Any) -> Optional[bytes]:
        """
        Converte um chart Plotly (dict JSON) para PNG bytes usando kaleido.
        ...
        try:
            ...
            # Ajuste básico de layout (opcional)
            try:
                fig.update_layout(width=1000, height=500)
            except Exception:
                pass

            # Exporta para PNG (kaleido)
            return fig.to_image(format="png", scale=2)

        except Exception as e:
            print(f"⚠️ Não foi possível converter gráfico Plotly em PNG (instale plotly+kaleido): {e}")
            return None

    def _add_chart_to_doc(self, doc: Document, chart_json: Any, title: Optional[str] = None):
        """Insere um gráfico Plotly no DOCX como imagem."""
        png_bytes = self._chart_json_to_png_bytes(chart_json)
        if not png_bytes:
            return
    ...
    def generate_docx_report(
        ...
        # Histórico Q/A (+ tabelas + gráficos quando apropriado)
        ...
        for i, question in enumerate(analysis["user_questions"], 1):
            ...
            if i <= len(analysis["assistant_responses"]):
                ...
                if resp["has_chart"]:
                    ...
                    # Inserir gráfico no DOCX
                    if resp.get("chart"):
                        self._add_chart_to_doc(doc, resp["chart"], title="Gráfico:")

        # Consultas SQL (+ tabelas + gráficos quando apropriado)
        if analysis["queries_executed"]:
            doc.add_page_break()
            ...
            for i, query in enumerate(analysis["queries_executed"], 1):
                ...
                # Gráfico (se existir)
                #if query.get("chart"):
                #    self._add_chart_to_doc(doc, query["chart"], title="Gráfico da consulta:")
