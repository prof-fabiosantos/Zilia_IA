...
async def read_root(request: Request):  # ← ADICIONA request: Request
    return {"status": "Zilla IA API está online!"}


@app.get("/health", summary="Status detalhado da API")
@limiter.limit("30/minute")
async def health_check(request: Request):  # ← ADICIONA request: Request
    """Retorna status detalhado da API e persistência de memória"""
    global memory_manager
    
    persistence_status = "ATIVO ✅" if memory_manager and memory_manager.is_using_persistent_storage() else "INATIVO ⚠️"
...
async def handle_chat(request: Request, question_request: QuestionRequest):  # ← ADICIONA request: Request
    """
    ...
    try:
        ...
        if memory_manager:
            ...
            if similar_results:
                ...
                threshold = memory_manager.get_threshold(question)
                print(f"📊 Threshold dinâmico para esta pergunta: {threshold:.2%}")
                ...
                if similarity >= threshold:
                    ...
                    print(f"⚠️ Similaridade {similarity:.2%} abaixo do threshold {threshold:.2%} - gerando nova resposta")
        ...
        if not any(keyword in sql_normalized for keyword in ['SELECT', 'EXEC', 'CALL']):
            response.summary = "Não encontrei dados, tente novamente, se não obter os dados novamente experiemente reformular a consulta."
        ...
        try:
            ...
            response.summary = "Desculpe! Não foi possível executar a consulta no banco de dados, reformule a sua consulta e tente novamente."
            return response
...
async def confirm_interaction(request: Request, confirm_request: ConfirmInteractionRequest):  # ← ADICIONA request: Request
    """
    Endpoint para confirmar se uma resposta foi útil.
    ...
    try:
        # Se o usuário marcou como útil, salva na memória
        if confirm_request.is_useful:
            memory_id = memory_manager.save_interaction(
                ...
                is_confirmed=True  # Já nasce confirmado (aprovado pelo usuário)
            )
            ...
            # Usuário marcou como não útil - apenas descarta
            print(f"❌ Resposta marcada como não útil (não foi salva na memória)")
...
@limiter.limit("30/minute")
async def memory_stats(request: Request):  # ← ADICIONA request: Request
    """
...
@limiter.limit("5/minute")  # Limite mais restritivo (relatórios são pesados)
async def generate_report(
    request: Request,
    report_request: dict  # Recebe {"messages": [...], "format": "docx" ou "json"}
):
    """
